 ...
 *
 * - create a multiline string similar to csv format and create a TabText object
 * with it - use line feed "\n" (ascii 10) as line separator and grave accent
 * "`" (ascii 96) as field separator - you can use some format codes, see
 * http://minecraft.gamepedia.com/Formatting_codes - DO NOT USE LOWERCASE CODES
 * OR BOLD FORMAT BECAUSE IT CAN BREAK SPACING
 *
 * - // example - multilineString = "PLAYER------`RATE------`RANK------\n"; -
 * multilineString += "�EJohn`10.01`1�R\n"; - multilineString += "Doe`-9.30`2";
 *
 * - TabText tt = new TabText(multilineString); - int numPages =
 * tt.setPageHeight(pageHeight); // set page height and get number of pages -
 * tt.setTabs(10, 18, ...); // horizontal tabs positions - tt.sortByFields(-2,
 * 1); // sort by second column descending, then by first - printedText =
 * tt.getPage(desiredPage, (boolean) monospace); // get your formatted page, for
 * console or chat area
 *
...
public class TabText {
     ...
     *
     * @param multilineString the string to use
     */
    public final void setText(String multilineString) {
     ...
     *
     * @param charsList a string with the chars to be added (careful with unicode or
